...
def _resolve_unitree_model_dir() -> str:
...
UNITREE_MODEL_DIR = _resolve_unitree_model_dir()
if not os.environ.get("UNITREE_MODEL_DIR"):
    print(f"[unitree] UNITREE_MODEL_DIR not set; defaulting to {UNITREE_MODEL_DIR}")
...
class UnitreeUsdFileCfg(sim_utils.UsdFileCfg):
    ...
    rigid_props = sim_utils.RigidBodyPropertiesCfg(
        disable_gravity=False, # 开启重力作用
        retain_accelerations=False, # 不保留上一时刻加速度（更物理一致）。
        linear_damping=0.0,
        angular_damping=0.0,
        max_linear_velocity=1000.0,
        max_angular_velocity=1000.0,
        max_depenetration_velocity=1.0, # 穿透修正速度上限，控制接触求解器分离穿透的力度
    )

    # solver_position_iteration_count=8: 位置约束迭代次数（提高接触/关节稳定性）
    # solver_velocity_iteration_count=4: 速度约束迭代次数（接触反弹/阻尼稳定性）
...
class UnitreeUrdfFileCfg(sim_utils.UrdfFileCfg):
    fix_base: bool = False
    activate_contact_sensors: bool = True
    replace_cylinders_with_capsules = True # 将 URDF 中的圆柱体碰撞换为胶囊体，提升稳定性与数值接触表现。
    joint_drive = sim_utils.UrdfConverterCfg.JointDriveCfg(
        gains=sim_utils.UrdfConverterCfg.JointDriveCfg.PDGainsCfg(stiffness=0, damping=0)
    ) # URDF 转换阶段的关节驱动配置，PD 增益设为零，避免在导入阶段注入控制；实际控制由后续 actuators 配置接管。
